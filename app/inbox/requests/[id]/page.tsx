"use client";
import { useState, use } from "react";
import Link from "next/link";
import { ArrowLeft, Ban, Check, Flag, X } from "lucide-react";
import { inbox } from "@/lib/demo-data";
import { messagingEligibility } from "@/lib/domain/messaging";

export default function RequestsPage({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = use(params);
  const req = inbox.find((x: any) => x.id === resolvedParams.id) || inbox.find((x: any) => x.kind === "request") || inbox[0];

  const person = req;
  const context = {
    mutualConnection: person.mutualConnection ?? false,
    sharedChallenge: person.sharedChallenge ?? false,
    sharedTeam: person.sharedTeam ?? false,
    acceptedRequest: person.acceptedRequest ?? false,
    pilotEnabled: person.pilotEnabled ?? false,
    blocked: false,
    priorDecline: false,
    senderAgeBand: "18_24" as const,
    recipientAgeBand: person.ageBand ?? "18_24",
  };
  const eligibility = messagingEligibility(context);

  const [decision, setDecision] = useState<string | null>(null);

  return (
    <div className="page-narrow">
      <Link href="/inbox?tab=requests" className="eyebrow">
        <ArrowLeft size={15} /> Inbox
      </Link>
      <header style={{ margin: "25px 0" }}>
        <p className="eyebrow">One introduction only</p>
        <h1 className="display-sm">MESSAGE<br/>REQUESTS.</h1>
      </header>

      {decision ? (
        <div className="empty-state">
          <Check size={35} />
          <h2 className="display-sm" style={{ fontSize: 38 }}>{decision}.</h2>
          <p>This request can't be repeatedly recreated. You can adjust your controls any time.</p>
        </div>
      ) : eligibility === "DENIED" ? (
        <div style={{ padding: 20, background: "var(--bg)", borderRadius: 8, margin: "20px 0" }}>
          <p className="muted" style={{ marginBottom: 16 }}>
            Messaging is not permitted for this user based on your settings.
          </p>
          <button
            className="button"
            onClick={() => setDecision("Blocked")}
            style={{ width: "100%" }}
          >
            Block user
          </button>
        </div>
      ) : (
        <article className="panel panel-pad">
          <div style={{ display: "grid", gridTemplateColumns: "70px 1fr", gap: 14 }}>
            <img
              src={req.avatar}
              alt=""
              style={{ width: 70, height: 70, objectFit: "cover", border: "var(--rule)" }}
            />
            <div>
              <h2 style={{ margin: 0 }}>{req.displayName}</h2>
              <p style={{ margin: "3px 0" }}>@{req.handle}</p>
              <span className="status">New request</span>
            </div>
          </div>
          <p className="notice" style={{ marginTop: 18 }}>
            {req.context}
          </p>
          <blockquote
            style={{
              margin: "20px 0",
              borderLeft: "5px solid var(--red)",
              paddingLeft: 15,
              lineHeight: 1.5,
            }}
          >
            " {req.preview} "</blockquote>
          <p
            className="muted"
            style={{ fontSize: 12 }}
          >
            {req.displayName} cannot send another message until you accept. Declining prevents this request from being recreated.</p>
          <div
            style={{
              display: "grid",
              gridTemplateColumns: "1fr 1fr",
              gap: 8,
              marginTop: 20,
            }}
          >
            <button
              className="button lime"
              onClick={() => setDecision("Accepted")}
            >
              <Check size={15} /> Accept
            </button>
            <button className="button" onClick={() => setDecision("Declined")}>
              <X size={15} /> Decline
            </button>
            <button
              className="button ghost"
              onClick={() => setDecision("Blocked")}
            >
              <Ban size={15} /> Block
            </button>
            <button className="button ghost" onClick={() => setDecision("Reported")}>
              <Flag size={15} /> Report
            </button>
          </div>
        </article>
      )}
    </div>
  );
}
